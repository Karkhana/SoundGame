...
int lastfreq = (int)frequency-6;


void setup()
{

  size(1024, 400, OPENGL);   // P2D, P3D, OPENGL
...
void draw()
{
  ...
  int y1 = (int)((frequency-6)/8.6+50);  // scale
  stroke (255, 255, 255, 64);
  line (y1, 0, y1, 400);  
  text (frequency-6+" Hz", y1+10, 80);//display the frequency in hertz
  
  text (maximum, 150, 130);
  if (maximum >= 1) { 
    int lastfreq = (int)frequency-6;
      textSize(50);


  text (lastfreq+" Hz", 150, 100);//display the frequency in hertz
    textSize(12);
  int y_last = (int)((lastfreq)/8.6+50);   // scale
  stroke (255, 255, 255, 255);  // pink
  line (y_last, 0, y_last, 400);
  
int x = y_last;
int y = 150;
rectMode(CENTER);
rect(x+100,y+100,x+20,y+100);
ellipse(x+100,y+70,x+60,y+60);
ellipse(x+81,y+70,x+16,y+32); 
ellipse(x+119,y+70,x+16,y+32); 
